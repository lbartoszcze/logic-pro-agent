...
// Per-track plugin insertion (Compressor / EQ / Saturator) is intentionally
// not exposed yet. The Inspector's audioPlugIn slot AXButtons report DISABLED
// and reject AXPress; pixel clicks via Cua's double_click pixel mode land
// in the slot region but Logic does not register them as "open plugin
// chooser." Inserting plugins programmatically requires either driving the
// Mixer panel slot AXButtons (which expose a private `Open plug-in menu`
// action that Cua's `click` does not currently surface) or shipping a
// preset Channel Strip Setting that already contains the plugin chain.
// Leaving this as a known gap rather than shipping a stub that pretends to
// insert when it actually misses.
