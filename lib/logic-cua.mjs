...
export function cuaSwap(pid, window_id, trackHint, patchName) {
  ...
  let rowMatch = tree.match(rowRe);

  if (!rowMatch) {
    const subcatRe = /\[(\d+)\] AXStaticText = "(?:downloadable )?([^"]+)" help="Patch category/g;
    const subcats = [];
    let m;
    while ((m = subcatRe.exec(tree)) !== null) subcats.push(parseInt(m[1]));
    for (const idx of subcats) {
      call("double_click", { pid, window_id, element_index: idx });
      sleepSec(0.5);
      tree = snapshot(pid, window_id);
      rowMatch = tree.match(rowRe);
      if (rowMatch) break;
    }
  }
  if (!rowMatch) throw new Error(`no Library entry for ${patchName} in any of this track's subcategories`);

  // The patch row may be virtually positioned below the viewport. Scroll the
  // patches AXScrollArea + retry up to 5 times; verify against Library title.
  const sa = tree.match(/\[(\d+)\] AXScrollArea actions=\[AXScrollLeftByPage/);
  for (let i = 0; i < 5; i++) {
    const probe = snapshot(pid, window_id);
    const pm = probe.match(rowRe);
    if (!pm) break;
    call("double_click", { pid, window_id, element_index: parseInt(pm[1]) });
    sleepSec(0.6);
    const t = snapshot(pid, window_id).match(/AXStaticText = "([^"]+)" id=_NS:137/);
    if (t && t[1].includes(patchName)) return `loaded ${patchName} on ${trackHint}`;
    if (sa) call("scroll", { pid, window_id, element_index: parseInt(sa[1]), by: "page", direction: "down", amount: 1 });
    sleepSec(0.3);
  }
  throw new Error(`tried to load ${patchName} on ${trackHint} but Library title did not match`);
}
