...
export function cuaSwap(pid, window_id, trackHint, patchName) {
  ...
  if (!rowMatch) {
    // Patch isn't in the currently-displayed subcategory. Walk every indexed
    // AXStaticText that's a "Patch category" (subcategories like "EDM Bass",
    // "Lead", etc.), click each, and re-look for the patch. First hit wins.
    const subcatRe = /\[(\d+)\] AXStaticText = "(?:downloadable )?([^"]+)" help="Patch category/g;
    const subcats = [];
    let m;
    while ((m = subcatRe.exec(tree)) !== null) subcats.push({ index: parseInt(m[1]), name: m[2] });
    for (const sc of subcats) {
      call("double_click", { pid, window_id, element_index: sc.index });
      sleepSec(0.5);
      tree = snapshot(pid, window_id);
