...
// Note 1 OVERLAPS note 2's onset by 30 ticks — on mono trap 808 patches
// (Tough 808, Heavy 808, etc.) this triggers the synth's portamento and
// produces a real pitch glide between the two notes, the way actual trap
// 808s slide. On poly patches it's just a tiny crossfade, also fine.
// Returns [{tick, length, pitch, vel}, ...] all relative to bar start.
export function trap808Bar(barIdx, root, beatTicks) {
  const out = [];
  const overlap = 30;
  out.push({ tick: 0, length: beatTicks * 2 + overlap, pitch: root, vel: 110 });
...
// Section labels for an N-bar arrangement, organised in 4-bar phrases:
//   intro  : drums + bass only          (first phrase)
//   verse  : full mix                   (odd middle phrases)
//   chorus : full mix + busier hats     (even middle phrases)
//   outro  : break — chords + bass      (last phrase)
// Short loops collapse: ≤4 bars all-verse; 8 = intro+chorus; 12 = intro+verse+chorus.
// Larger arrangements cycle verse/chorus across the middle phrases.
...
// Pseudo-sidechain compression via MIDI CC7 (volume) automation. For each
// kick hit in the bar, schedule a quick volume duck (30) → recovery (105) so
// the bass "pumps" out of the way of the kick. Same audible effect as real
// sidechain compression but pure MIDI, no plugin needed.
//   kickPat: 16-step pattern string ("x..x..x...")
//   barOffset: tick offset of bar 0 of this bar
//   sixteenth: ticks per 16th
// Returns array of {tick, value} CC7 events.
export function sidechainCC7(kickPat, barOffset, sixteenth) {
  const events = [];
  const recoverMs = 6;
  for (let i = 0; i < 16; i++) {
    if (kickPat[i] !== "x") continue;
    const t = barOffset + i * sixteenth;
    events.push({ tick: t, value: 30 });
    events.push({ tick: t + recoverMs * sixteenth, value: 105 });
  }
  return events;
}
